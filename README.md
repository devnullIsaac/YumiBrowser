...
3. **The host is designed to be auditable by non-experts.** A core goal of Yumi Browser is that **anyone with minimal programming skill should be able to walk through the source code and verify the security claims for themselves**. The host is written in plain C11, the binding modules are organized one capability per file, and the WebAssembly API surface is documented header by header. You do not need to be a cryptographer or a kernel engineer to confirm, with your own eyes, that no networking import lets a webapp reach outside its group. That property — self-auditability by ordinary developers — is treated as a feature, not an afterthought.
