...
| **Crypto** | libsodium exclusively |
| **Handle** | Opaque `gr_registrar_t *` — DB connection, cached header, ~60 prepared statements, join state |
| **API** | 101 public functions, all returning `gr_error_t` (except boolean queries and `void` teardown) |

**20 source files**, single-subsystem-per-file:

| File | Responsibility |
|------|---------------|
| `lifecycle.c` | `gr_create`, `gr_open`, `gr_close` — handle allocation, DB init, teardown |
| `db.c` | Schema (10 tables), header load/save, ~60 prepared statements, column validation, migrations |
...
| `audit.c` | Hash-chained, Ed25519-signed audit log — append, list, count, chain verification, fork detection, retention enforcement |
| `crypto.c` | Registrar signing/verification, data signing, AEAD encryption (epoch key), per-peer sealed boxes (X25519) |
...
| `invite.c` | Invite ticket creation (`GRINV` v2), parsing, invalidation, usage tracking |
...
| `identity.c` | Ed25519 keypair generation, peer ID derivation (`BLAKE2b-256(pk)`), X25519 conversion, secure wipe |
...
|-------|---------|---------|
| `gr_header` | 16 | Single-row envelope: group_id, name, type, version, timestamps, epoch_id, retention policy, owner\_id/key, signer\_id/key, signature, hash |
| `gr_peers` | 12 | peer_id (PK), public_key, sign_key, ip, port, status, role_id, joined_at, removed_at, last_seen, removed_reason, removed_by |
| `gr_roles` | 6 | role_id (PK), name, permissions (bitmask), sign_key, created_at, modified_at. Max 50 roles |
| `gr_webapps` | 5 | hash (PK), name, version, added_at, added_by |
| `gr_servers` | 8 | id_hash (PK), type (signaling/rebroadcast), ip, port, public_key, service_hash, content key pair |
| `gr_epochs` | 5 | epoch_id (PK), epoch_key, created_at, expired_at, created_by |
| `gr_audit_log` | 10 | entry_hash (PK), timestamp, change_type, actor_id, target_id, signature, registrar_version, detail, prev_hash, timestamp_ns |
...
All crypto is libsodium. No algorithm negotiation, no fallbacks.

| Primitive | Algorithm | Usage |
|-----------|-----------|-------|
| **Peer identity** | Ed25519 keypair (32-byte pk, 64-byte sk) | Every peer has one; `peer_id = BLAKE2b-256(public_key)` |
| **Registrar signing** | Ed25519 detached | Owner/authorized signer signs a canonical header hash |
| **Audit log** | BLAKE2b-256 hash + Ed25519 detached signature | Each entry hashes `prev_hash ‖ timestamp ‖ change_type ‖ actor_id ‖ target_id ‖ version ‖ detail`, then signs the hash |
| **Group encryption** | XChaCha20-Poly1305 AEAD | Uses the current epoch key; nonce prepended; epoch_id as additional data |
| **Per-peer encryption** | X25519 sealed boxes | Ed25519 keys converted to X25519 for DH-based sealed box encrypt/decrypt |
| **Hashing** | BLAKE2b-256 (32 bytes) | Content hashes, peer IDs, invite tokens, group icon verification |

**Canonical header** for signing: `group_id ‖ group_type ‖ group_name ‖ version ‖ created_at ‖ epoch_id ‖ retention{3 fields} ‖ owner_id ‖ owner_sign_key ‖ signer_id ‖ signer_sign_key`. The BLAKE2b hash of this is Ed25519-signed.
...
- **Signed**: Each entry's hash is Ed25519-signed by the actor who performed the mutation.
- **21 change types**: From peer joins/kicks to epoch rotations to icon changes (see `gr_change_type_t`).
...
2. **Attestation collection** — The joiner connects to existing peers. Each peer signs an attestation: `group_id ‖ owner_id ‖ owner_sign_key ‖ version ‖ nonce` (132 bytes). Submitted via `gr_join_submit_peer_header()`.
...
| **Invite** | `GRINV` | 2 | Lightweight ticket — group identification, bootstrap peers, signaling servers, verification token |
...
- Verifies each entry's BLAKE2b hash (including `prev_hash` chain link) and Ed25519 signature
...
- **Security**: verification_token, inviter public key, expiry timestamp
- **Integrity**: Ed25519 signature over all preceding bytes
...
Epoch keys are 32-byte symmetric keys (XChaCha20-Poly1305) for group-wide encryption. On rotation:
...
Encryption prepends `nonce ‖ epoch_id_le32` to ciphertext. Decryption tries the specified epoch_id first, then iterates active epochs (key rotation mid-conversation).
...
Supports static images (PNG, JPEG, WebP) and video loops (MP4, WebM), up to 512×512 at 10 MB. Video icons require a static frame fallback. The icon's BLAKE2b content hash enables efficient sync — peers compare hashes before fetching the full blob. Icons are excluded from the serialized registrar to avoid bloating sync traffic.
...
Each server has: id_hash, ip, port, public_key, service_hash, content key pair. Requires `GR_PERM_EDIT_SERVERS`.
...
| `test_crypto.c` | Signing, verification, encryption/decryption, sealed boxes, key derivation |
...
5. **Crypto is non-negotiable** — Ed25519 identities, BLAKE2b hashing, XChaCha20-Poly1305 encryption, all via libsodium. No algorithm negotiation.
