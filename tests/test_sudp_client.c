/*
 * test_sudp_client.c - Comprehensive Secure UDP tests: invite→join→attest, triple- and dual-hybrid handshakes, multi-channel data, kicked peers, simultaneous open.
 ...
 *   5.  Sustained 3-second session — throughput, stability, state.
                     ...
                     uint16_t pa, uint16_t pb)
{
  ...
  cb.recv_cb = recv_cb;
  cb.state_cb = state_cb;
  cb.user = sb_s;

  *out_a = NULL;
  *out_b = NULL;
  if (yumi_sudp_client_create(out_a, &ca) != 0)
    return -1;
  if (yumi_sudp_client_create(out_b, &cb) != 0)
  {
    yumi_sudp_client_destroy(*out_a);
    *out_a = NULL;
    return -1;
  }
  if (yumi_sudp_client_connect(*out_a, id_b->peer_id) != 0)
  {
    yumi_sudp_client_destroy(*out_a);
    *out_a = NULL;
    yumi_sudp_client_destroy(*out_b);
    *out_b = NULL;
    return -1;
  }
...
static void test_registrar_setup(void)
{
  ...
  /* Create group */
  T(gr_create(&g_reg, ":memory:", "TestSUDP", GR_GROUP_PRIVATE, &g_owner) == GR_OK, "gr_create");
...
/* ════════════════════════════════════════════════════════════════════════
 *  Section 5: Sustained 3-second session
 *
...
{
  SEC("5  Sustained 3-second session");
  ...
  T(yumi_sudp_client_get_state(sa) == YUMI_SUDP_ESTABLISHED,
    "A still ESTABLISHED after 3s");
  T(yumi_sudp_client_get_state(sb) == YUMI_SUDP_ESTABLISHED,
    "B still ESTABLISHED after 3s");
